...
@export var path_calculator : PathCalculator
var rooms = []
...
func build():
	...
	if path_calculator:
		path_calculator.build(rooms)
	else:
		push_warning("LevelGenerator: no PathCalculator assigned.")
...
func spawn_room(scene : PackedScene, pos : Vector3, dir : Vector3, from_room : Room = null):
	...
	if from_room:
		room.parent_room = from_room
		room.depth = from_room.depth + 1
		from_room.connected_rooms.append(room)
