...
func spawn_room(scene: PackedScene, pos: Vector3, dir: Vector3, parent_room: Room) -> bool:
    ...
    # Convert to_center from local to global BEFORE moving the room
    var door_global_offset = room.to_global(door_info["to_center"]) - room.global_position
    room.global_position = pos - door_global_offset
